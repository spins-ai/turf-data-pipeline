#!/usr/bin/env python3
"""
Script 86 — Scraping SmartForm.co.uk
Source : smartform.co.uk (UK form database, speed ratings)
Collecte : form guides UK, speed ratings, sectional times, race cards
CRITIQUE pour : UK Speed Ratings, Sectional Analysis, Form Database Features
"""
...
log = setup_logging("86_smartform")
...
BASE_URL = "https://www.smartform.co.uk"



def scrape_smartform_day(session, date_str):
    """Scraper les racecards et speed ratings SmartForm pour un jour donné."""
    ...
    url = f"{BASE_URL}/racecards/{date_url}"
    ...
    for section in soup.find_all(["div", "section", "article"], class_=True):
        ...
        if any(kw in classes.lower() for kw in ["race", "meeting", "card", "fixture",
                                                  "event", "racecard"]):
            record = {
                "date": date_str,
                "source": "smartform",
    ...
    # --- Tables: runners, speed ratings, form ---
    for table in soup.find_all("table"):
        ...
        for row in rows[1:]:
            ...
            if cells and len(cells) >= 2:
                ...
                for cell in cells:
                    sr_match = re.search(r'(\d{2,3})', cell)
                    if sr_match and 30 <= int(sr_match.group(1)) <= 150:
                        entry["speed_rating"] = sr_match.group(1)
                        break
                records.append(entry)

    # --- Speed ratings sections ---
    for div in soup.find_all(["div", "section", "span"], class_=True):
        classes = " ".join(div.get("class", []))
        if any(kw in classes.lower() for kw in ["speed", "rating", "sr", "figure",
                                                  "rprs", "topspeed", "sectional"]):
    ...
    for div in soup.find_all(["div", "section", "table"], class_=True):
        classes = " ".join(div.get("class", []))
        if any(kw in classes.lower() for kw in ["form", "history", "previous", "last-runs",
                                                  "record", "performance", "past"]):
            text = div.get_text(strip=True)
    ...
    for div in soup.find_all(["div", "section", "table"], class_=True):
        classes = " ".join(div.get("class", []))
        if any(kw in classes.lower() for kw in ["sectional", "split", "timing", "furlong"]):
    ...
    for div in soup.find_all(["div", "p", "article", "section"], class_=True):
        classes = " ".join(div.get("class", []))
        text = div.get_text(strip=True)
        if any(kw in classes.lower() for kw in ["tip", "analysis", "comment", "verdict",
                                                  "selection", "preview", "nap"]):
    ...
    for script in soup.find_all("script"):
        ...
        for m in re.finditer(r'JSON\.parse\s*\(\s*[\'"](.+?)[\'"]\s*\)', script_text, re.DOTALL):
            try:
                ...
                pass
        for m in re.finditer(r'window\[?[\'"]?(__\w+|raceData|formData|speedData|runnerData)[\'"]?\]?\s*=\s*(\{.+?\});',
    ...
    for el in soup.find_all(attrs=lambda attrs: attrs and any(
            ...
            ["speed", "rating", "horse", "runner", "race", "form", "sectional", "odds"])
            for k in attrs)):
...
def scrape_smartform_results(session, date_str):
    """Scraper les résultats SmartForm pour un jour donné."""
...
def main():
    parser = argparse.ArgumentParser(description="Script 86 — SmartForm Scraper (UK form, speed ratings)")
    ...
    log.info("=" * 60)
    log.info("SCRIPT 86 — SmartForm Scraper")
    ...
    while current <= end_date:
        ...
        records = scrape_smartform_day(session, date_str)
        if records:
            for rec in records:
        ...
        results = scrape_smartform_results(session, date_str)
        if results:
            for rec in results:
                append_jsonl(output_file, rec)
                total_records += 1
...
def aggregate_cache_to_jsonl():
    ...
    _aggregate_cache_to_jsonl(CACHE_DIR, os.path.join(OUTPUT_DIR, "smartform_data.jsonl"), logger=log)
