...
def scrape_harness_day(page, date_str):
    ...
    au_date = f"{parts[2]}/{parts[1]}/{parts[0]}"

    urls_to_try = [
        f"{BASE_URL}/racing/results?date={date_str}",
        f"{BASE_URL}/racing/results/{date_str}",
        f"{BASE_URL}/results?date={au_date}",
        f"{BASE_URL}/racing/meetings/{date_str}",
    ]
    ...
    for url in urls_to_try:
        ...
        # -- Meeting headers --
        for div in soup.find_all(["div", "section", "header", "h2", "h3"], class_=True):
            ...
            if any(kw in classes.lower() for kw in ["meeting", "venue", "track",
                ...
                if text and 3 < len(text) < 500:
                    ...
                    if venue_match:
                        record["venue"] = venue_match.group(1)
                    records.append(record)

        # -- Horse/driver profile links --
        ...
        for div in soup.find_all(["div", "span", "p"], class_=True):
            classes = " ".join(div.get("class", []))
            text = div.get_text(strip=True)
            if any(kw in classes.lower() for kw in ["race-detail", "conditions",
                                                      "stake", "distance", "class"]):
                if text and 5 < len(text) < 500:
                    ...
                    stake_match = re.search(r'\$\s*([\d,]+)', text)
                    if stake_match:
                        record["stake_aud"] = stake_match.group(1).replace(",", "")
                    records.append(record)

        # -- Sectional times --
        for div in soup.find_all(["div", "table", "section"], class_=True):
            classes = " ".join(div.get("class", []))
            if any(kw in classes.lower() for kw in ["sectional", "split", "quarter",
                                                      "mile-rate", "time"]):
                text = div.get_text(strip=True)
...
def main():
    parser = argparse.ArgumentParser(
        description="Script 94 -- Harness Racing Australia Scraper")
