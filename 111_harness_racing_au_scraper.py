...
def extract_race_links(soup, date_str):
    """Extract links to individual races or meetings."""
    links = set()
    for a in soup.find_all("a", href=True):
        href = a["href"]
        if re.search(r'(race/|meeting/|result|field)', href, re.I):
...
def extract_data_attributes(soup, date_str):
    ...
    records = []
    keywords = ["horse", "runner", "driver", "trainer", "odds", "sp",
                "result", "position", "barrier", "handicap", "form",
                "gait", "pacer", "trotter"]
