...
def extract_race_links(soup):
    """Extract links to individual race entries / results pages."""
    links = set()
    for a in soup.find_all("a", href=True):
        href = a["href"]
        if re.search(r'/(race|entries|results|card|pp)/', href, re.I):
...
def extract_runners_table(soup, date_str, race_url=""):
    """Extract runner data from entries or results tables."""
    records = []
...
def extract_data_attributes(soup, date_str):
    ...
    records = []
    keywords = ["horse", "runner", "jockey", "trainer", "odds", "ml",
                "result", "position", "speed", "figure", "pace", "post"]
