#!/usr/bin/env python3
"""
Script 154 — Racing Post Free Racecards Scraper (Playwright)
Source : racingpost.com (free content only)
Collecte : Free racecards with Official Rating (OR), form figures, basic stats
URL patterns :
  /racecards/{date}                          -> liste des courses du jour
  /racecards/{date}/{venue}/{time}/{race_id} -> carte de course individuelle
  /results/{date}                            -> resultats du jour
CRITIQUE pour : Official Ratings (OR), Form Figures, Draw Data, Free Racecards
...
SCRIPT_NAME = "154_racingpost_free"
...
log = setup_logging("154_racingpost_free")
...
def extract_official_ratings(soup, date_str):
    """Extract Official Ratings (OR) from racecard pages."""
    records = []
    for el in soup.find_all(["div", "span", "td"], class_=True):
        classes = " ".join(el.get("class", []))
        if any(kw in classes.lower() for kw in ["or", "official-rating", "rating",
                                                   "rpr", "ts", "topspeed",
                                                   "rp-rating", "horse-rating"]):
            text = el.get_text(strip=True)
            if text and 1 < len(text) < 300:
                record = {
                    "date": date_str,
                    "source": "racingpost_free",
                    "type": "official_rating",
                    "contenu": text[:200],
                    "classes_css": classes,
...
def extract_form_figures(soup, date_str):
    """Extract form figures and recent performance data."""
    records = []
    for el in soup.find_all(["div", "span", "td", "a"], class_=True):
        classes = " ".join(el.get("class", []))
        if any(kw in classes.lower() for kw in ["form", "form-figures", "recent-form",
                                                   "formfig", "horse-form",
                                                   "last-runs", "rp-form"]):
            text = el.get_text(strip=True)
            if text and 1 < len(text) < 500:
                ...
                form_match = re.search(r'([0-9PFU/\-]{2,15})', text)
                if form_match:
...
def extract_draw_data(soup, date_str):
    """Extract draw/stall position data from racecards."""
    records = []
    for el in soup.find_all(["div", "span", "td"], class_=True):
        classes = " ".join(el.get("class", []))
        if any(kw in classes.lower() for kw in ["draw", "stall", "barrier",
                                                   "saddle-cloth", "cloth-number"]):
            text = el.get_text(strip=True)
            if text and 0 < len(text) < 200:
                ...
                draw_match = re.search(r'(\d{1,2})', text)
                if draw_match:
                    record["draw_number"] = draw_match.group(1)
                records.append(record)
    return records
...
def scrape_day_racecards(page, date_str):
    """Scrape Racing Post free racecards page for a given date."""
    ...
    records.extend(extract_embedded_json_data(soup, "racingpost_free", date_str=date_str))
    records.extend(extract_scraper_data_attributes(soup, "racingpost_free", date_str=date_str))
    records.extend(extract_official_ratings(soup, date_str))
    records.extend(extract_form_figures(soup, date_str))
    records.extend(extract_draw_data(soup, date_str))
    records.extend(extract_runners_table(soup, "racingpost_free", date_str=date_str, race_url=url))
...
def scrape_race_detail(page, race_url, date_str):
    """Scrape individual racecard page for detailed runner data."""
    ...
    # Spotlight / verdict sections
    for div in soup.find_all(["div", "p", "section"], class_=True):
        classes = " ".join(div.get("class", []))
        if any(kw in classes.lower() for kw in ["spotlight", "verdict", "tip",
                                                   "comment", "analysis",
                                                   "selection", "diomed"]):
...
def scrape_day_results(page, date_str):
    """Scrape Racing Post results page for a given date."""
...
def main():
    parser = argparse.ArgumentParser(
        description="Script 154 — Racing Post Free Racecards Scraper (OR, form, stats)"
    )
    ...
    log.info("=" * 60)
    log.info("SCRIPT 154 — Racing Post Free Racecards Scraper (Playwright)")
    ...
    output_file = os.path.join(OUTPUT_DIR, "racingpost_free_data.jsonl")
