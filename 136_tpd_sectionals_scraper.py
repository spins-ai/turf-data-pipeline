#!/usr/bin/env python3
"""
Script 136 — Total Performance Data (TPD) Sectionals Scraper (Playwright)
Source : totalperfdata.com
Collecte : sectional times, stride data, finishing speed from free section
URL patterns :
  /free-sectionals/            -> free sectional times index
  /free-sectionals/{date}      -> sectional times for a given date
  /race/{race_id}/sectionals   -> race-level sectional breakdown
  /stride-data/                -> stride frequency/length data
CRITIQUE pour : Sectional Times, Stride Analysis, Finishing Speed (TODO 7I)
...
SCRIPT_NAME = "136_tpd_sectionals"
...
BASE_URL = "https://www.totalperfdata.com"
MAX_RETRIES = 3
...
def extract_sectional_tables(soup, date_str):
    """Extract sectional time data from tables on the page."""
    records = []
...
def extract_stride_data(soup, date_str):
    """Extract stride frequency and stride length data."""
    records = []
    for el in soup.find_all(["div", "span", "td", "section"], class_=True):
        classes = " ".join(el.get("class", []))
        if any(kw in classes.lower() for kw in ["stride", "frequency", "length",
                                                   "cadence", "biomech"]):
            ...
            if text and 1 < len(text) < 500:
                ...
                # Try to extract numeric stride values
                stride_match = re.search(r'(\d+\.?\d*)\s*(m|metres?|Hz|spm|strides?)', text, re.I)
                if stride_match:
                    record["stride_value"] = stride_match.group(1)
                    record["stride_unit"] = stride_match.group(2)
                records.append(record)
    return records


def extract_finishing_speed(soup, date_str):
    """Extract finishing speed / final furlong data."""
    records = []
    for el in soup.find_all(["div", "span", "td", "p"], class_=True):
        classes = " ".join(el.get("class", []))
        text = el.get_text(strip=True)
        if any(kw in classes.lower() for kw in ["finish", "speed", "final",
                                                   "closing", "last-furlong"]):
            if text and 1 < len(text) < 500:
                record = {
                    "date": date_str,
                    "source": "tpd",
                    "type": "finishing_speed",
                    "contenu": text[:300],
                    "classes_css": classes,
                    "scraped_at": datetime.now().isoformat(),
                }
                speed_match = re.search(r'(\d+\.?\d*)\s*(mph|kph|m/s|s)', text, re.I)
                if speed_match:
                    record["speed_value"] = speed_match.group(1)
                    record["speed_unit"] = speed_match.group(2)
                records.append(record)
    return records
...
def scrape_free_sectionals_index(page, date_str):
    """Scrape the TPD free sectionals index page for a given date."""
    cache_file = os.path.join(CACHE_DIR, f"sectionals_{date_str}.json")
    if os.path.exists(cache_file):
        with open(cache_file, "r", encoding="utf-8") as f:
            return json.load(f)

    url = f"{BASE_URL}/free-sectionals/{date_str}"
    html = navigate_with_retry(page, url)
    if not html:
        return None

    # Save raw HTML
    html_file = os.path.join(HTML_CACHE_DIR, f"sectionals_{date_str}.html")
...
def scrape_race_sectionals(page, race_url, date_str):
    """Scrape individual race page for detailed sectional breakdowns."""
    ...
    # Pace / position in running sections
    for div in soup.find_all(["div", "section", "p"], class_=True):
        classes = " ".join(div.get("class", []))
        if any(kw in classes.lower() for kw in ["pace", "position", "tracking",
                                                   "running-style", "split",
                                                   "furlong-by-furlong"]):
...
def scrape_stride_page(page, date_str):
    """Scrape the stride data overview page."""
    cache_file = os.path.join(CACHE_DIR, f"stride_{date_str}.json")
    if os.path.exists(cache_file):
        with open(cache_file, "r", encoding="utf-8") as f:
            return json.load(f)

    url = f"{BASE_URL}/stride-data/{date_str}"
    html = navigate_with_retry(page, url)
    if not html:
        return None
...
def main():
    parser = argparse.ArgumentParser(
        description="Script 136 — TPD Sectionals Scraper (sectional times, stride data)"
    )
    ...
    log.info("=" * 60)
    log.info("SCRIPT 136 — TPD Sectionals Scraper (Playwright)")
    ...
    output_file = os.path.join(OUTPUT_DIR, "tpd_sectionals_data.jsonl")
    ...
    try:
        ...
        while current <= end_date:
            ...
            # Scrape free sectionals index
            result = scrape_free_sectionals_index(page, date_str)
            ...
            if result:
                ...
                # Scrape individual race sectional pages (limit to avoid overload)
                for race_url in result.get("race_links", [])[:15]:
                    detail = scrape_race_sectionals(page, race_url, date_str)
                    if detail:
            ...
            # Also scrape stride data page
            stride_data = scrape_stride_page(page, date_str)
            if stride_data:
                for rec in stride_data:
                    append_jsonl(output_file, rec)
                    total_records += 1
