#!/usr/bin/env python3
"""
Script 143 — Equine Edge Scraper (Playwright)
Source : equineedge.com
Collecte : Form analysis, trainer patterns, jockey statistics, performance trends
URL patterns :
  /form-guide/{date}        -> daily form guides
  /trainer-stats/            -> trainer performance patterns
  /jockey-stats/             -> jockey statistics
  /horse/{horse-slug}/       -> individual horse form profile
  /results/{date}            -> results with form analysis
CRITIQUE pour : Form Analysis, Trainer Patterns, Jockey Stats
...
log = setup_logging("143_equine_edge")

BASE_URL = "https://www.equineedge.com"
MAX_RETRIES = 3
...
def extract_form_analysis(soup, date_str):
    """Extract form analysis data from page content."""
    records = []
...
def extract_trainer_patterns(soup, date_str):
    """Extract trainer performance patterns and statistics."""
    records = []
    for el in soup.find_all(["div", "section", "table", "article"], class_=True):
        classes = " ".join(el.get("class", []))
        if any(kw in classes.lower() for kw in ["trainer", "pattern", "stats",
            ...
            if text and 5 < len(text) < 3000:
                ...
                name_match = re.search(r'(?:trainer|entra.neur)\s*:?\s*([A-Z][a-zA-Z\s\'-]+)', text)
                if name_match:
                    record["trainer_name"] = name_match.group(1).strip()
                # Try to extract strike rate
                sr_match = re.search(r'(\d{1,3}(?:\.\d{1,2})?)\s*%', text)
                if sr_match:
                    record["strike_rate"] = sr_match.group(1)
                records.append(record)
    return records


def extract_jockey_stats(soup, date_str):
    """Extract jockey statistics and performance data."""
    records = []
    for el in soup.find_all(["div", "section", "table", "article"], class_=True):
        classes = " ".join(el.get("class", []))
        if any(kw in classes.lower() for kw in ["jockey", "rider", "stats",
                                                   "performance", "record",
                                                   "booking", "riding"]):
            text = el.get_text(strip=True)
            if text and 5 < len(text) < 3000:
                ...
                name_match = re.search(r'(?:jockey|rider)\s*:?\s*([A-Z][a-zA-Z\s\'-]+)', text)
                if name_match:
                    record["jockey_name"] = name_match.group(1).strip()
                sr_match = re.search(r'(\d{1,3}(?:\.\d{1,2})?)\s*%', text)
                if sr_match:
                    record["strike_rate"] = sr_match.group(1)
                records.append(record)
...
def scrape_form_guide_page(page, date_str):
    """Scrape the form guide listing page for a given date."""
    ...
    html_file = os.path.join(HTML_CACHE_DIR, f"form_{date_str}.html")
    ...
    records.extend(extract_embedded_json_data(soup, "equine_edge", date_str=date_str))
    records.extend(extract_scraper_data_attributes(soup, "equine_edge", date_str=date_str))
    records.extend(extract_form_analysis(soup, date_str))
    records.extend(extract_trainer_patterns(soup, date_str))
    records.extend(extract_jockey_stats(soup, date_str))
    records.extend(extract_runners_table(soup, "equine_edge", date_str=date_str))
...
def scrape_race_detail(page, race_url, date_str):
    """Scrape individual race/form page for detailed analysis."""
...
def scrape_results_day(page, date_str):
    ...
    records.extend(extract_scraper_data_attributes(soup, "equine_edge", date_str=date_str))
    records.extend(extract_form_analysis(soup, date_str))
    records.extend(extract_runners_table(soup, "equine_edge", date_str=date_str, race_url=url))
...
def main():
    parser = argparse.ArgumentParser(
        description="Script 143 — Equine Edge Scraper (form analysis, trainer patterns)"
    )
    ...
    log.info("=" * 60)
    log.info("SCRIPT 143 — Equine Edge Scraper (Playwright)")
    ...
    output_file = os.path.join(OUTPUT_DIR, "equine_edge_data.jsonl")
    ...
    try:
        ...
        while current <= end_date:
            ...
            # Scrape form guide page
            result = scrape_form_guide_page(page, date_str)
            ...
            if result:
                ...
                # Scrape individual race/form pages
