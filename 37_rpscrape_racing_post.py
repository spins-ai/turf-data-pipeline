...
OUTPUT_DIR = "output/37_racing_post"
        ...
        if resp.status_code == 200 and len(resp.text) > 2000:
            ...
            for race in race_cards:
                ...
                for runner in runners:
                    cells = runner.find_all(["td", "span", "div"])
...
def main():
    log.info("=" * 60)
    log.info("SCRIPT 37 — Racing Post Results (FR courses)")
    log.info("=" * 60)
    
    rotate_session()
    all_records = []
    output_file = os.path.join(OUTPUT_DIR, "racing_post_fr.json")
    ...
    for course_name, course_id in FR_COURSES.items():
        ...
        while current < end:
            ...
            if collected % 20 == 0 and collected > 0:
                log.info(f"    {collected} pages, {len(all_records)} records")
    ...
    log.info(f"TERMINÉ: {collected} pages, {len(all_records)} records Racing Post")
